...
class TargetPIDControllerNode(Node):
    def __init__(self):
        ...
        #self.tracked_rigid_bodies = yaml.safe_load(yaml_file)
    ...
    def set_target(self, msg):
        ...
        # Need to be stored for later use in the control loop
    ...
    def create_subscribers(self):
        for i in range(self.num_agents):  
            ...
            self.robot_subscribers[topic_name] = self.create_subscription(PointStamped, topic_name, self.set_target, 10)
    ...
    def handle_rigid_bodies(self, msg):
        self.get_logger().info('RIGID BODIES CALLED')
    ...
    def control_loop(self):
        ...
        if self.controller.target is not None:
            #self.get_logger().info('INSIDE IF STATEMENT')
            for robot_name, pose_data in self.poses.items():
                ...
                x = position[0]
                y = position[1]
                orientation_val = orientation[0]
                pose = x, y, orientation_val
                ...
                if pose is not None:
                    ...
                    msg.x = x
                    msg.y = y
                    msg.orientation = orientation_val
